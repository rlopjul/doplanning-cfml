...
}}));return a;}function showAlertMessage(c,a){if($("#alertContainer span").length!=0){$("#alertContainer span").remove();}if(a==true){$("#alertContainer").attr("class","alert alert-success");
}else{$("#alertContainer").attr("class","alert alert-danger");}$("#alertContainer button").after("<span>"+c+"</span>");var b=getMaxZIndex();$("#alertContainer").css("zIndex",b+1);
$("#alertContainer").fadeIn("slow");setTimeout(function(){hideAlertMessage();},9500);}function hideAlertMessage(){$("#alertContainer").fadeOut("slow",function(){$("#alertContainer span").remove();
...
}function loadModal(a){$("body").modalmanager("loading");$modal.load(a,"",function(){$modal.modal({width:740,backdrop:"static"});});}function hideModal(a){$(a).modal("hide");
}function hideDefaultModal(){$modal.modal("hide");}function postModalForm(c,a,b,d){$("body").modalmanager("loading");$.ajax({type:"POST",url:a,data:$(c).serialize(),dataType:"json",success:function(g,e){if(e=="success"){var f=g.message;
