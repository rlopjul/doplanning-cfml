var curAreaId="undefined";var areaWithLink=false;var currentTab="#tab1";var disableNextTabChange=false;function windowHeight(){var a=document.documentElement;
return a.clientHeight;}function getURLParameterFromPath(a,b){return decodeURIComponent((new RegExp("[?|&]"+a+"=([^&;]+?)(&|#|;|$)").exec(b)||[,""])[1].replace(/\+/g,"%20"))||null;
}function getFilename(a){return a.substring(a.lastIndexOf("/")+1);}function getMaxZIndex(){var a=Math.max.apply(null,$.map($("body > *"),function(b,c){if($(b).css("position")=="absolute"||$(b).css("position")=="fixed"){return parseInt($(b).css("z-index"))||1;
...
});}function showAlertModal(b){var a={msg:b,error:false};$("#alertModal").html(tmpl("tmpl-alert-modal",a));$("#alertModal").modal({width:650,backdrop:"static"});
}function showAlertErrorModal(b){var a={msg:b,error:true};$("#alertModal").html(tmpl("tmpl-alert-modal",a));$("#alertModal").modal({width:650,backdrop:"static"});
}function loadModal(a){$("body").modalmanager("loading");var b=generateRandom();if(a.indexOf("?")==-1){a=a+"?n"+b;}else{a=a+"&n"+b;}$modal.load(a,"",function(){$modal.modal({width:740,backdrop:"static"});
});}function hideModal(a){$(a).modal("hide");}function hideDefaultModal(){$modal.modal("hide");}function postModalForm(c,a,b,d){$("body").modalmanager("loading");
$.ajax({type:"POST",url:a,data:$(c).serialize(),dataType:"json",success:function(g,e){if(e=="success"){var f=g.message;hideDefaultModal();openUrl(b,d);
$("body").modalmanager("removeLoading");showAlertMessage(f,g.result);}else{alert(e);}}});}function postModalFormTree(b,a){$("body").modalmanager("loading");
